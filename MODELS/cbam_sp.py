...
class SpatialGateBU(nn.Module):
    def __init__(self):
        super(SpatialGateBU, self).__init__()
        kernel_size = 7
...
class CBAMSP2(nn.Module):
    def __init__(self, gate_channels, reduction_ratio=16, pool_types=['avg', 'max'], no_spatial=False):
        super(CBAMSP2, self).__init__()
    ...
    def forward(self, x, sp_att_pre):
        ...
        if not self.no_spatial:
            x_out, sp_att = self.SpatialGateBU(x_out, sp_att_pre)
        return x_out, sp_att
...
class SpatialGate(nn.Module):
    ...
    def forward(self, x):
        ...
        return x * scale, x_out

class CBAMSP1(nn.Module):
    def __init__(self, gate_channels, reduction_ratio=16, pool_types=['avg', 'max'], no_spatial=False):
        super(CBAMSP1, self).__init__()
        self.ChannelGate = ChannelGate(gate_channels, reduction_ratio, pool_types)
        self.no_spatial=no_spatial
        if not no_spatial:
